...
def main_BOSSVA(proportion, word_size, window_size):
...
#    clf = BOSSVS(word_size = word_size, window_size = window_size)
    clf = BOSSVS(word_size = word_size, window_size = window_size)
...
#print(main_BOSSVA(0.7,2,28 ))

def best_proportion_and_window_size_for_BOSSVA():
    word_size = 2
    record_df = pd.DataFrame(index=range(5,18),columns=range(word_size,26))
    for i in range(5,18):
        proportion = i/20
        data_record_window_size = []
        for j in range(word_size,26):
            window_size = j
            score = main_BOSSVA(proportion, word_size, window_size)
    ...
    record_df.to_csv(path_result+'/best_proportion_and_wondow_size_for_BOSSVA.csv')
    return record_df

best_proportion_and_window_size_for_BOSSVA()
